...
async function withCommunityName<T extends { communityId: string | null }>(
  user: T,
): Promise<T & { communityName: string | null }> {
  if (!user.communityId) {
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post("/auth/register", async (request, reply) => {
    ...
    return reply.status(201).send({
      user: { ...user, trustBand: getTrustBand(user.trustScore) },
    });
  });
  // post /auth/login
  ...
  app.post("/auth/login", async (request, reply) => {
    ...
    return reply.status(200).send({
      ...
      user: {
        ...userWithCommunity,
        trustBand: getTrustBand(userWithCommunity.trustScore),
      },
    });
  });
  ...
  app.get("/auth/me", { preHandler: requireAuth }, async (request, reply) => {
    ...
    return reply.status(200).send({
      user: {
        ...userWithCommunity,
        trustBand: getTrustBand(dbUser.trustScore),
      },
    });
  });
