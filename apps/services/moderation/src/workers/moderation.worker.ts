...
async function processJob(job: ModerationJob): Promise<void> {
  ...
  const decision: "approved" | "flagged" =
    anyFlagged || maxScore >= threshold ? "flagged" : "approved";
  ...
  if (decision === "approved") {
    try {
      ...
      console.warn(
        `[Moderation] Kafka post.created publish failed (non-fatal):`,
        (kafkaErr as Error).message,
      );
    }
  }
  ...
  try {
      ...
      messages: [
        {
          ...
          value: JSON.stringify({
            userId: job.authorId,
            eventType:
              decision === "approved" ? "post_approved" : "post_removed",
    ...
    console.warn(
      `[Moderation] Kafka user.events publish failed (non-fatal):`,
      (kafkaErr as Error).message,
    );
  }

  console.log(
    `[Moderation] Post ${job.postId} → ${decision} (score : ${maxScore.toFixed(2)})`,
  );
}
